...
schedules_listing_page_html = requests.get(Constants.SCHEDULE_LISTING_PAGE_URL, headers=headers).text

# extract date of last update.

last_updated_text = extract_last_update_text(schedules_listing_page_html)
print("Last update date: " + last_updated_text)
...
first_specialty_text_pos = schedules_listing_page_html.find(Constants.FIRST_SPECIALTY_TEXT)
part_time_education_text_pos = schedules_listing_page_html.find(Constants.PART_TIME_EDUCATION_TEXT)

schedules_listing_page_html = schedules_listing_page_html[first_specialty_text_pos : part_time_education_text_pos]
...
schedule_links = list(filter(lambda link: not link[1].startswith("Про"), schedule_links))
...
for link in schedule_links:
    ...
    schedule_download_link = Constants.GOOGLE_DRIVE_FILE_DOWNLOAD_LINK_TEMPLATE.format(file_id)
...
for schedule_download_link in tqdm(schedule_download_links, desc="Downloading files", unit="file"):
    link_href = schedule_download_link[0]
    link_text = schedule_download_link[1]
    
    schedule_pdf = requests.get(link_href)    
    schedule_pdf_filename = link_text + ".pdf"
    
    with open(os.path.join(Constants.PATH_TO_OUTPUT_DIRECTORY, schedule_pdf_filename), 'wb') as f:
